...
CalibrationWizard(){
	...
	Static CalibrationOnChar := 0, CalibrationOnChat := 0, CalibrationOnInventory := 0
	Static CalibrationOnVendor := 0, CalibrationOnDiv := 0, CalibrationDetonate := 0
	Static CalibrationOnMenu := 0, CalibrationEmpty := 0, CalibrationOnStash := 0
	Static CalibrationOnDelveChart := 0
	Static varOnChar := "", varOnChat := "", varOnMenu := "", varOnInventory := ""
	Static varOnVendor := "", varOnStash := "", varOnLeft := "", varOnDiv := ""
	...
	WizardGui.Add("CheckBox", "Checked vCalibrationOnInventory     xp   y+10      wp h20", "Inventory Open")
	WizardGui.Add("CheckBox", "Checked vCalibrationOnVendor      xp   y+10      wp h20", "Vendor Trade Open")
	WizardGui.Add("CheckBox", "vCalibrationOnDiv             xp   y+10      wp h20", "Divination Trade Open")
	WizardGui.Add("CheckBox", "vCalibrationDetonate          xp   y+10      wp h20", "Detonate Shown")
	WizardGui.Add("CheckBox", "Checked vCalibrationOnMenu        xp+140 ys       wp h20", "Talent Menu Open")
	WizardGui.Add("CheckBox", "Checked vCalibrationEmpty         xp   y+10      wp h20", "!EMPTY! Inventory Open")
	WizardGui.Add("CheckBox", "Checked vCalibrationOnStash       xp   y+10      wp h20", "Stash Open")
	WizardGui.Add("CheckBox", "vCalibrationOnDelveChart        xp   y+10      wp h20", "Delve Chart Open")

	btnRun := WizardGui.Add("Button", "x100 y240 w160 h30", "Run Wizard")
	btnRun.OnEvent("Click", RunWizard)
	btnCancel := WizardGui.Add("Button", "x+20 yp wp hp", "Cancel Wizard")
	btnCancel.OnEvent("Click", WizardClose)
	...
	RunWizard(*)
	{
		...
		CalibrationOnVendor    := saved.CalibrationOnVendor
		CalibrationOnDiv       := saved.CalibrationOnDiv
		CalibrationDetonate    := saved.CalibrationDetonate
		CalibrationOnMenu      := saved.CalibrationOnMenu
		CalibrationEmpty       := saved.CalibrationEmpty
		CalibrationOnStash     := saved.CalibrationOnStash
		CalibrationOnDelveChart := saved.CalibrationOnDelveChart
	...
	{
		WizardGui := Gui("+AlwaysOnTop")
		WizardGui.Add("Button", "w1 h1")
		WizardGui.Add("Edit",, SampleTT . EmptySampleTT)
		btnSave := WizardGui.Add("Button",, "Save Samples")
		btnSave.OnEvent("Click", SaveWizardResults)
		btnAbort := WizardGui.Add("Button", "x+20", "Abort Samples")
		btnAbort.OnEvent("Click", WizardClose)
		WizardGui.OnEvent("Close", WizardClose)
		WizardGui.OnEvent("Escape", WizardClose)
		WizardGui.Show()
	}
