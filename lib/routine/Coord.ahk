...
CoordCommand(*){
	...
	If (Rect)
	{
		...
		Notify.Show(Clipboard, "`nThis information has been placed in the clipboard`nCalculation Took " (T1 := A_TickCount - T1) " MS for " (T_Area := ((Rect.X2 - Rect.X1 + 1) * (Rect.Y2 - Rect.Y1 + 1))) " Pixels`n" Round(T1 / T_Area,3) " MS per pixel",5)
