Class SevenZip {
  ...
  backup(){
    ...
    RunWait(A_ComSpec " /c `"" "`"" This.ExeFile "`" " This.AddArgs " `"" ToZip "`" `"" This.SourceFile "`" | `"" This.Mtee "`" /D/T/+ `"" This.LogOutput "`"" "`"",, "Hide", &ZipPID)
  }
  restore(date){
    loc := A_ScriptDir "\backup\" date ".7z"
    If FileExist(loc){
      ExtArgs := "x `"" loc "`" -o`"" A_ScriptDir "`" -y"
      RunWait(A_ComSpec " /c `"" "`"" This.ExeFile "`" " ExtArgs " | `"" This.Mtee "`" /D/T/+ `"" This.LogOutput "`"" "`"",, "Hide", &ZipPID)
    }
  }
  install(branch){
    ...
    RunWait(A_ComSpec " /c " MoveArgs,,"hide")
    RemoveArgs := "rmdir /s /q " subfolder
    RunWait(A_ComSpec " /c " RemoveArgs,,"hide")
  }
}
